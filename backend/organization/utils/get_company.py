...
def func_get_company(request_data, token):
	try:
		response={}
		...
		if isAuthorized:
			...
			if len(getCompany) > 0:
				...
				response['message'] = 'No company found in database.'
				response["statuscode"] = 400
		else:
			logs["data"]["data_fields"] = [comapnyInfo["client_id"]]
			logs["data"]["status_message"] = 'Only SuperUser can fetch the company.'
			response['message'] = 'Only SuperUser can fetch the company.'
			response["statuscode"] = 400
