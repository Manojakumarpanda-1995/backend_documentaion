...
def func_edit_company(request_data, token):
	try:
		response={}
		...
		if isAuthorized:
			...
			if len(getCompany) > 0:
				...
				logs["data"]["status_message"] = "Company data updated successfully."
			...
			logs["data"]["status_message"] = "You are not authorized to edit company details."

			response['message'] = "You are not authorized to edit company details."
			response["statuscode"] = 400
