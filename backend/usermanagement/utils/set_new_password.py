...
from usermanagement.utils.hash import generate_passwords  # encryption,decryption
...
def func_set_new_password(request_data, token):
    try:
        ...
        if isAuthorized:
            ...
            if generate_passwords(apiParamsInfo["old_password"]) == old_password:
                apiParamsInfo["updated_by"] = curr_user

                # getUser.password = encryption(apiParamsInfo["new_password"])
                getUser.password = generate_passwords(apiParamsInfo["new_password"])
                getUser.save()
