...
def validate_email(email):
    try:
        ...
        logging.info("Email_vlaidation==>", str(e))


def func_edit_user(request_data, token):
    try:
        response = {}
        ...
        for key, value in apiParamsInfo.items():
            if key in [f.name for f in curr_user._meta.get_fields()]:
                changed_values.append((getattr(curr_user, key), apiParamsInfo[key]))
                setattr(curr_user, key, apiParamsInfo[key])
